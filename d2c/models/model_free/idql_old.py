...
class IDQLAgent(BaseAgent):
    ...
    def __init__(
        ...
        super(IDQLAgent, self).__init__(**kwargs)
    ...
    def _build_p_loss(self, batch: Dict) -> Tuple[Tensor, Dict]:
        ...
        # IDQL 策略损失：标准的扩散模型去噪 MSE 损失
        # 不再依赖 q - v 的优势值
        p_loss = self._p_fn.get_diffusion_loss(s, a)

        info = collections.OrderedDict()
        info['bc_diffusion_loss'] = p_loss
        return p_loss, info
    ...
    def _build_test_policies(self) -> None:
        min_q_fn = lambda s, a: torch.min(self._q_fns[0](s, a), self._q_fns[1](s, a))

        policy = policies.IDQLArgmaxPolicy(
            p_fn=self._p_fn,
            q_fn=min_q_fn, # 传入最小Q值计算函数
            num_samples=64,
            device=self._device,
        )
    ...
    def _get_modules(self) -> utils.Flags:
        ...
        def p_net_factory():
            return networks.DiffusionActor(
                observation_space=self._observation_space,
                action_space=self._action_space,
                num_res_blocks=3,
                device=self._device,
